...
describe('API - Autenticacao', () => {
  ...
  describe('POST /api/auth/register', () => {
    ...
    it('bloqueia cadastro sem campos obrigatorios', async () => {
      const response = await request(app)
        .post('/api/auth/register')
        .send({ email: 'sem.nome@example.com' });

      expect(response.status).to.equal(400);
      expect(response.body.message).to.equal('Nome, e-mail e senha são obrigatórios.');
    });
  ...
  describe('POST /api/auth/login', () => {
    ...
    it('bloqueia login sem campos obrigatorios', async () => {
      const response = await request(app)
        .post('/api/auth/login')
        .send({ email: 'usuario@receitasdavo.com' });

      expect(response.status).to.equal(400);
      expect(response.body.message).to.equal('E-mail e senha são obrigatórios.');
    });
